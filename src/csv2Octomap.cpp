...
namespace csv_2_octomap {
	...
	void Csv2Octomap::inflateMap(float robot_radius){
		...
		octomap::KeyRay keyRay_1,keyRay_2;
		...
		for (octomap::OcTree::iterator it = octree->begin(octree->getTreeDepth()),end = octree->end(); it != end; ++it){
			...
			if(point.z() >= 0){
				octomap::point3d point_infl_1(point.x() + robot_radius, point.y() + robot_radius, point.z());
				octomap::point3d point_infl_2(point.x() + robot_radius, point.y() - robot_radius, point.z());
				if (octree->computeRayKeys(point_infl_1, point_infl_2, keyRay_1))
					inflate_cells.insert(keyRay_1.begin(), keyRay_1.end());

				octomap::point3d point_infl_3(point.x() - robot_radius, point.y() + robot_radius, point.z());
				octomap::point3d point_infl_4(point.x() - robot_radius, point.y() - robot_radius, point.z());
				if (octree->computeRayKeys(point_infl_3, point_infl_4, keyRay_2))
					inflate_cells.insert(keyRay_2.begin(), keyRay_2.end());
			}
		}
